...
const isDrinkwardSuiteTriple = (roomNumber, dormId) => {
    const drinkwardTripleInDormPullExceptions = [
        "123C",
        "124C",
        "221C",
        "222C",
...
function BumpModal() {
    ...
    const [roomsWhoCanAlternatePull, setRoomsWhoCanAlternatePull] = useState([
        ["Example Room Number", "Example Room UUID"],
    ]);
    ...
    useEffect(() => {
        ...
        if (selectedSuiteObject) {
            ...
            for (let room of otherRooms) {
                if (
                    room.roomNumber !== selectedItem &&
                    room.maxOccupancy === 1 &&
                    room.occupant1 !== 0 &&
                    room.pullPriority.pullType === 1
                ) {
                    console.log("room", room);
                    otherOccupants.push([room.occupant1, room.roomUUID]);
                }
                if (
                    selectedRoomObject.maxOccupancy === 2 &&
                    room.roomNumber !== selectedItem &&
                    room.maxOccupancy === 2
                ) {
                    ...
                    if (data.error) {
                        ...
                        if (data.error === "One or more of the proposed occupants is already in a room") {
                            ...
                            if (
                                ...
                                data.occupants.every(
                                    (occupant) => selectedRoomObject.roomUUID === getRoomUUIDFromUserID(occupant)
                                )
                            ) {
                            ...
                            data.error === "One or more of the proposed occupants is not preplaced" &&
                            data.occupants
                        ) {
    ...
    const canIBePulled = () =>
        performRoomAction(2, peopleWhoCanPullSingle.find((person) => person[0] === Number(pullMethod))[1]);
            ...
            fetch(`${process.env.REACT_APP_API_URL}/rooms/clear/${roomUUID}`, {
                ...
                .then((response) => {
                    ...
                    return handleBlocklistCheck(response).then((isBlocklisted) => {
                        if (isBlocklisted) {
                            setLoadingClearRoom(false);
                            if (personIndex !== -1) {
                                setLoadingClearPerson(
                                    loadingClearPerson.map((item, itemIndex) =>
                                        itemIndex === personIndex ? false : item
                                    )
                                );
                            }
                            resolve(false);
                ...
                .then((data) => {
                    ...
                    if (data.error) {
                        ...
                        if (personIndex !== -1) {
                            setLoadingClearPerson(
                                loadingClearPerson.filter((_, itemIndex) => itemIndex !== personIndex)
                            );
                            setPeopleAlreadyInRoom(
                                peopleAlreadyInRoom.filter((_, itemIndex) => itemIndex !== personIndex)
                            );
                            setShowModalError("");
                        }
                        if (closeModalBool) {
    ...
    const cancelClearRoom = () => {
        ...
        if (roomToClearPersonIndex !== -1) {
            setLoadingClearPerson(
                loadingClearPerson.map((item, itemIndex) => (itemIndex === roomToClearPersonIndex ? false : item))
            );
        }
    };
    ...
    const executeClearRoom = () => {
        ...
        if (roomToClearPersonIndex === -1) {
            ...
            setLoadingClearPerson(
                loadingClearPerson.map((item, itemIndex) => (itemIndex === roomToClearPersonIndex ? true : item))
            );
        }
            ...
            <div className="modal-card">
                <header className="modal-card-head">
                    <p className="modal-card-title">
                        {selectedRoomObject.pullPriority.isPreplaced
                            ? "Can't edit preplaced room"
                            : `Edit Room ${selectedItem}`}
                    </p>
                ...
                <section className="modal-card-body">
                    ...
                    {selectedSuiteObject &&
                        ...
                        selectedSuiteObject.genderPreferences.length > 0 && (
                            <div className="notification is-warning" style={{ marginBottom: "15px" }}>
                                ...
                                <p className="mt-2">
                                    <strong>Note:</strong> Knowingly violating these gender preferences is an{" "}
                                    <strong>Honor Code violation</strong>.
                                </p>
                            </div>
                        )}
                                                ...
                                                <Select
                                                    ...
                                                    styles={{
                                                        ...
                                                        option: (provided, state) => ({
                                                            ...
                                                            color: state.isFocused ? "#ffffff" : "var(--text-color)",
                                                        }),
                            ...
                            <div>
                                ...
                                <div className="select">
                                    <select value={pullMethod} onChange={handlePullMethodChange}>
                                        ...
                                        {(selectedRoomObject.maxOccupancy === 1 ||
                                            isDrinkwardSuiteTriple(selectedItem, selectedRoomObject.dorm)) &&
                                            peopleWhoCanPullSingle.map((item, index) => (
                                                <option key={index} value={item[0]}>
                                                    Pulled by {getNameById(item[0])}
                                                </option>
                                            ))}
                                        {selectedSuiteObject.alternative_pull &&
                                            roomsWhoCanAlternatePull.map((room, index) => (
                                                <option key={index} value={`Alt Pull ${room[1]}`}>
                                                    Pull w/ 2nd best of {selectedRoomObject.roomNumber} and {room[0]}
                                                </option>
                                            ))}
                                        {selectedSuiteObject.can_lock_pull && (
                    ...
                    {peopleAlreadyInRoom.map((person, index) => (
                        <div key={index} style={{ marginTop: "5px" }} className="field">
                            <button
                                className={`button is-danger ${loadingClearPerson[index] ? "is-loading" : ""}`}
                                onClick={() => {
                                    initiateRoomClear(getRoomUUIDFromUserID(person), false, index);
                                }}
                            >
                                Clear {getNameById(person)}'s existing room
                            </button>
                        </div>
                    ))}
                        ...
                        <div className="modal is-active">
                            ...
                            <div className="modal-card">
                                    ...
                                    {clearConfirmationStep === 1 ? (
                                        <div>
                                            <div className="notification is-warning">
                                                <p className="has-text-weight-bold">
                                                    Warning: Clearing a room is a dangerous operation
                                                </p>
                                                <p>
                                                    This will remove ALL occupants from the room and cannot be undone.
                                                </p>
                                        ...
                                        <div>
                                            <div className="notification is-danger">
                                                ...
                                                <p className="mt-3 has-text-weight-bold">
                                ...
                                <footer className="modal-card-foot" style={{ justifyContent: "space-between" }}>
                                    <button className="button" onClick={cancelClearRoom}>
                                        Cancel
                                    </button>
                                    {clearConfirmationStep === 1 ? (
                                        <button className="button is-warning" onClick={confirmClearRoom}>
                                            Proceed to Confirmation
                                        </button>
                                    ) : (
                                        <button className="button is-danger" onClick={executeClearRoom}>
                                            Yes, Clear Room
                                        </button>
                                    )}
                                </footer>
                ...
                >
                    ...
                    >
                        ...
                        <button
                            ...
                            disabled={
                                clearRoomStats.remainingClears <= 0 ||
                    ...
                    {clearRoomStats.isBlocklisted ? (
                        ...
                        >
                            Your account has been temporarily restricted due to excessive room clearing. Please contact
                            an administrator.
                        </div>
                        ...
                        >
                            Warning: You are approaching the daily limit for room clearing. Further clearing may result
                            in account restriction.
                            {clearRoomStats.resetsInMinutes && (
                                <div style={{ marginTop: "5px" }}>
                                    Limits reset at midnight Pacific Time which is (
                                    {formatTimeUntilReset(clearRoomStats.resetsInMinutes)})
                                </div>
                            )}
                        </div>
                    ) : (
