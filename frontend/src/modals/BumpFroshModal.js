...
const BumpFroshModal = () => {
    ...
    const filteredRooms =
        ...
        rooms.filter(
            (room) =>
                !room.HasFrosh &&
                room.FroshRoomType === selectedRoomObject.froshRoomType &&
                dormMapping[room.Dorm] === activeTab
        );
            ...
            <div className="modal-card">
                ...
                <section className="modal-card-body">
                    {(jwtDecode(credentials).email === "tlam@g.hmc.edu" ||
                        ...
                        <button className="button is-warning mb-3" onClick={() => removeFrosh(selectedRoomObject)}>
                            Remove Frosh
                        </button>
                    )}

                    <label className="label">
                        {filteredRooms.length > 0 ? "Bump these frosh to a new room" : "Unbumpable frosh"}
                    </label>
                            ...
                            <select value={targetRoom} onChange={(event) => handleSelectChange(event)}>
                                ...
                                {rooms &&
                                    filteredRooms.map((room, index) => (
                                        <option key={index} value={room.RoomUUID}>
                                            Room {room.RoomID}
                                        </option>
                                    ))}
                            </select>
